...
# --------------------------------------------------------
    ...
    def QSOBHaccrete_distribution(self, G):
        ...
        for i in range(MERGER_NUM):
            ...
            thisPlot = plt.scatter(np.log10(G.Mvir[w] * 1.0e10), np.log10(G.QSOBHaccrete[w,i]), marker='o', s=20., c=t, cmap = cm.jet_r, alpha=0.8, label='Stars')
...
# --------------------------------------------------------
    ...
    def QSOluminosity_function(self, G):
        ...
        outputFile = OutputDir + '19.QSOLuminosityFunction' + OutputFormat
        plt.savefig(outputFile)  # Save the figure
        print 'Saved file to', outputFile
        plt.close()
