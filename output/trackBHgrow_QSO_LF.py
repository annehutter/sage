...
# --------------------------------------------------------
    ...
    def QSOBHaccrete_distribution(self, G):
        ...
        for i in range(MERGER_NUM):
            ...
            thisPlot = plt.scatter(np.log10(G.Mvir[w] / self.Hubble_h*1.e10), np.log10(G.QSOBHaccrete[w,i] / self.Hubble_h*1.e10), marker='o', s=20., c=t, cmap = cm.jet_r, alpha=0.8, label='Stars')
            thisPlot.set_clim(vmin=0,vmax=1)

        plt.ylabel(r'$\log \Delta M_{BH}$    $[\mathrm{M}_{\odot}]$')  # Set the y...
...
# --------------------------------------------------------
    ...
    def QSOBHaccrete_function(self, G):
        ...
        for i in range(MERGER_NUM):
            BHaccrete = BHaccrete + G.QSOBHaccrete[w,i] / self.Hubble_h * 1.e10
...
# --------------------------------------------------------
    ...
    def QSO_distribution(self, G):
        ...
        outputFile = OutputDir + '18.QSODistribution_z' + str(G.z) + OutputFormat
        plt.savefig(outputFile)  # Save the figure
        print 'Saved file to', outputFile
        plt.close()
...
# --------------------------------------------------------
    ...
    def QSOluminosity_function(self, G):
        ...
        rBondi = Grav_const/cs**2*G.BlackHoleMass[w]/self.Hubble_h*2.e33*1.e10
        ...
        for i in range(MERGER_NUM):
            ...
            BHadd = np.where(BHaccrete>0. ,(BH+1.e2)/tEdd_yr*(1.-efficiency)/efficiency*np.exp(time/tEdd*(1.-efficiency)/efficiency), 0.)
        ...
        M1450luminosity = luminosity/tmp*(nu1450/nuBreak)**-alpha1
        M1450luminosity2 = luminosity2/tmp*(nu1450/nuBreak)**-alpha1
        M1450luminosity3 = luminosity3/tmp*(nu1450/nuBreak)**-alpha1

        # M1450luminosity = luminosity/tmp2 * (nu1050/nu1450)**-0.5 * (nu912/nu1050)**-alpha * (nu1450/nu912)**-alpha
        # M1450luminosity2 = luminosity2/tmp2 * (nu1050/nu1450)**-0.5 * (nu912/nu1050)**-alpha * (nu1450/nu912)**-alpha
        # M1450luminosity3 = luminosity3/tmp2 * (nu1050/nu1450)**-0.5 * (nu912/nu1050)**-alpha * (nu1450/nu912)**-alpha
        ...
        MUV1450_2 = -2.5*np.log10(M1450luminosity2/(4.*np.pi*lumDistance**2))-48.6-5.*np.log10(lumDistance/(10.*pc_cm))
        MUV1450_3 = -2.5*np.log10(M1450luminosity3/(4.*np.pi*lumDistance**2))-48.6-5.*np.log10(lumDistance/(10.*pc_cm))

        print luminosity
        print luminosity/tmp
        ...
        (counts2, binedges2) = np.histogram(MUV1450_2, range=(mi, ma), bins=NB)
        xaxeshisto2 = binedges2[:-1] + 0.5 * binwidth

        (counts3, binedges3) = np.histogram(MUV1450_3, range=(mi, ma), bins=NB)
        xaxeshisto3 = binedges3[:-1] + 0.5 * binwidth
...
# --------------------------------------------------------
    ...
    def check_merger_snapnum(self, G):
        ...
        outputFile = OutputDir + '20.CheckMergerSnapNum_z' + str(G.z) + OutputFormat
        plt.savefig(outputFile)  # Save the figure
        print 'Saved file to', outputFile
        plt.close()
